...
def decide(S: GameState, M: GameMap, P: Paths, turn: int) -> Actions:
    ...
    enemy_hq : Building = building_at(M.opp_hq)
    my_side : Side = M.my_side
    enemy_side : Side = M.my_side.opposite
    my_warriors : list[Warrior] = [w for w in S.warriors if w.id.side == my_side]
    enemy_warriors : list[Warrior] = [w for w in S.warriors if w.id.side == enemy_side]
